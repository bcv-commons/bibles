...
- **`core/`** — the original CDN-generation pipeline: versification
  ...
  DBT↔helloAO id crosswalk, and CDN publishing (`publish-dbt.sh`).
- **`comparison-and-research/`** — the PKF/DBT/helloAO version-identity
  comparison pipeline: fetches, char-/word-level text comparison,
  taxonomy diagnosis, and the two generators that produce
  `catalog-index.json` / `catalog-overlap.json` (see `../doc/`).
- **`paths.py`** — shared path constants both of the above import from,
  instead of hardcoding `data/`/`internal-data/`/`export/` string literals
  in each script. See its module docstring for the tracked-vs-gitignored
  rationale.
...
```bash
.venv/bin/python pipeline/comparison-and-research/generate_catalog_index.py
.venv/bin/python pipeline/comparison-and-research/generate_catalog_overlap.py
```
...
- `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET`
  ...
  `pipeline/comparison-and-research/pull_align_cache.py` use `rclone`
  against this)
